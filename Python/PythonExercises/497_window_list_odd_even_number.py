...
def show_odd_even_numbers():
    ...
    for entry in number_entries:
        try:
            ...
            if user_number % 2 == 0:
                even_numbers.append(user_number)
            else:
    ...
    result_window.title("Odd and even numbers")
    result_window.geometry("500x200")

    # Display total score
    tk.Label(
        result_window,
        text=f"Odd numbers: {odd_numbers}\nEven numbers: {even_numbers}",
    ).pack(pady=5, padx=20)
...
root = tk.Tk()
root.title("Odd and Even calculator")
