...
Важно: and/or возвращают не литералы True/False, а один из операндов (тот, на котором остановилось вычисление).
Отсюда идиома x = value or default - "взять value, а если оно ложное (None, 0, '', [], {} и т.п.) - взять default".
"""

# result.get("history") возвращает None, если ключа нет - or подставляет {} вместо None:
result = {"status": "ok"}
history = result.get("history") or {}
print("or как значение по умолчанию -", history)  # {}

# Если бы значение было истинным, or его и вернул бы, до правого операнда дело не дошло бы:
result = {"status": "ok", "history": [1, 2, 3]}
history = result.get("history") or {}
print("or вернул левый операнд -", history)  # [1, 2, 3]
"""
Другое объяснение:
NOT в логике тоже, что и смена знака в арифметике
AND аналог умножения (*)
OR аналог сложения (+)
Соответственно и приоритеты те же.
