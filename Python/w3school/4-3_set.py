...
переменная - временная переменная, принимает значения из последовательности
последовательность - список, строка или range, по которому идёт перебор
выражение - то, что попадёт во множество (обычно зависит от переменной)
условие (необязательное) - фильтр: во множество попадут только элементы, для которых условие истинно
"""

# множество квадратов чисел от 0 до 9
squares = {x ** 2 for x in range(10)}  # создаём множество квадратов чисел
print(squares)  # Порядок не гарантируется, например: {0, 1, 64, 36, 4, 9, 16, 81, 49, 25}

# множество чётных чисел от 0 до 9
evens = {x for x in range(10) if x % 2 == 0}  # фильтруем только чётные числа
