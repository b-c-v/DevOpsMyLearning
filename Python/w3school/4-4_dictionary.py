...
pop()           Removes the element with the specified key and returns the value associated with this key
...
{ключ: значение for элемент in коллекция if условие}
    ...
    ключ: значение - что вы кладёте в новый словарь на каждой итерации
    for элемент in коллекция - цикл, который перебирает исходные данные (список, словарь, множество и т.д.)
    if условие - необязательная часть, фильтрующая элементы
"""
...
print(f"Dictionary comprehension: {double_dict1}")
# Добавить условие (например, удваивать только чётные значения):
double_dict1 = {k: v * 2 for (k, v) in dict1.items() if v % 2 == 0}
