...
# ***def имя_функции(параметр: тип) -> возвращаемый_тип:***
...
# : (Parameter Type Hint Annotation / аннотация параметров)
def is_even(number: int):  # принимает аргумент типа int (хотя на самом деле может принимать и другие типы, так как Python не строго типизированный язык)
    if number % 2 == 0:
        return True
    else:
        return False

# -> (Return Type Hint Annotation / аннотация возвращаемого значения)
...
# Согласно PEP 484, аннотацию параметра и аннотацию возвращаемого значения можно и обычно нужно использовать вместе в одном определении функции:
